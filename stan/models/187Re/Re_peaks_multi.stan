...
parameters {
  ...
  vector[N_det]<lower=0> Along;
  vector[N_det]<lower=0> lamlong;
  real<lower=0> Ashort;
  real<lower=0> lamshort;
...
transformed parameters {
  ...
  vector[N_det]<lower=0> DeltaAshort=Ashort-Along;
  vector[N_det]<lower=0> Deltalamshort=lamshort-lamlong;
}
...
model {
  ...
  if (prior == 0) {
    ...
    for (k in N_det){
      vector[N_window] response = gauss_plus_double_exp(window_x, 0, sigma, lamlong[k], lamshort, Along[k], Ashort);
