...
transformed data {
  ...
  int N_peaks = 6;
  vector[N_peaks] x0 = head(to_vector([2047, 1842, 414.2, 333.5, 49.9, 26.3]),
                            N_peaks);
  vector[N_peaks] g = head(to_vector([13.2, 6.0, 5.4, 5.3, 3.0, 3.0]),
                           N_peaks);
  vector[N_peaks] h = head(to_vector([1, 0.0526, 0.2329, 0.0119, 0.0345, 0.0015]),
                           N_peaks);
  h = h / sum(h);
}
...
model {
  ...
  if (prior == 0) {
    counts ~ poisson(spectrum(full_x, window_x, sigma / dx, bkg, m_nu, Q, x0,
                              g, h, N_peaks)
                              * N_ev);
  }
}
