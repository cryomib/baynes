...
transformed data {
  ...
  vector[6-N_peaks] g_true = tail(to_vector([6.0, 13.2, 5.4, 5.3, 3.0, 3.0]), 6-N_peaks);
...
model {
  ...
  if (prior == 0) {
    counts ~ poisson(spectrum(full_x, window_x, sigma/dx, bkg, p_m, p_Q, append_row(x0, x0_true), append_row(g, g_true), append_row(h, h_true), N_peaks)*N_ev);
  }
}

generated quantities {
  array[N_bins] int counts_rep = poisson_rng(spectrum(full_x, window_x, sigma/dx, bkg, p_m, p_Q, append_row(x0, x0_true), append_row(g, g_true), append_row(h, h_true), N_peaks) * N_ev);
}