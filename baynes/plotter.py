...
class MatplotlibHelper:
    """Help creating and managing matplotlib figures."""
    ...
    def save_figures(self, titles=None):
        ...
        for title in titles:
            if title in self.figures:
                self.figures[title].savefig(
                    f"{self.output_dir}{title}{self.format}", bbox_inches="tight"
                )
    ...
    @modify_figure
    def update_legend(
        self, last_labels=None, edgecolor="white", bbox_to_anchor=(1.2, 0.8), **lgd_kws
    ):
        """
    ...
    def plot(self, *args, **kwargs):
        ...
        if self.save:
            fig.savefig(
                f"{self.output_dir}{self.current_title}{self.format}",
                bbox_inches="tight",
            )
        return ax
...
class FitPlotter(MatplotlibHelper):
    ...
    def add_fit(self, fit, fit_title=None, rep_names=None):
        ...
        if isinstance(fit, CmdStanMCMC):
            ...
            if rep_names is None:
                rep_names = ["_rep"]
            rep_variables = [
                col for col in all_variables if any(gen in col for gen in rep_names)
            ]
            stan_variables = [
                var
                for var in all_variables
                if var not in method_variables + rep_variables
            ]
    ...
    def single_fit_plot(func):
        """Wrap functions for plots from only one fit."""

        def wrapper(self, parameters="all_stan", legend=True, **kwargs):
            """
            Wrap plotting function with single-fit capability.
            ...
            if n_plots == 1:
                ...
                fig.set_size_inches(
                    self.fig_scale * n_cols * self.fig_ratio, self.fig_scale * n_rows
                )
    ...
    def multi_fit_plot(func):
        """Wrap functions for plots from multiple fits."""
        ...
        def wrapper(
            ...
            **kwargs,
            ...
            """
            Wrap plotting function with multi-fit capability.
            ...
            parameters = self.validate_parameters(parameters)
            if df is None:
                draws_df = self.draws_df(fit_titles, parameters, inc_warmup=inc_warmup)
            else:
                draws_df = df
            ...
            if func.__name__ != "pair_grid":
                ...
                if n_plots > 1:
                    f.set_size_inches(
                        self.fig_scale * self.fig_ratio * n_cols * norm,
                        self.fig_scale * n_rows * norm,
                    )
            else:
                f.set_size_inches(
                    self.fig_scale * (len(parameters)) ** 0.3,
                    self.fig_scale * (len(parameters)) ** 0.3,
                )
    ...
    def convergence_plot(
        ...
        **kwargs,
        ...
        """Plot the chains trace data for a single fit."""
        ax = figure.subplots(
            1, 2, width_ratios=[2.5, 1], sharey=True, gridspec_kw={"wspace": wspace}
        )
        ...
        if fit.metadata.cmdstan_config["save_warmup"]:
            ax[0].axvspan(
                0, fit.num_draws_warmup, color="gray", alpha=0.4, lw=0, label="Warmup"
            )
        ax[0].set_xlim(0)
    ...
    def pair_grid(
        ...
        **kwargs,
        ...
        if legend:
            grid.add_legend(
                label_order=grid.hue_names, title="", bbox_to_anchor=(0.9, 0.6)
            )
        return grid
    ...
    def dis_plot(
        ...
        **kwargs,
        ...
        displot = sns.displot(
            ...
            hue=hue,
            col=col,
            col_wrap=min(len(parameters), self.col_wrap),
            height=self.fig_scale,
            **kwargs,
        )
    ...
    def kde_plot(self, df, parameters, hue="fit", col="variable", **kwargs):
        ...
        def informative_kde(
            ...
            ax = sns.kdeplot(
                x, fill=False, bw_adjust=bw_adjust, color=color, label=label
            )
            kdeline = ax.lines[-1]
            ...
            ax.vlines(
                middle,
                0,
                np.interp(middle, x_data, y_data),
                color=median_color,
                ls=":",
                linewidth=1.5,
                label="median",
            ...
            ax.fill_between(
                ...
                color=color,
                alpha=0.4,
                label=str(percs[0]) + "-" + str(percs[2]) + "%",
            )
            ax.fill_between(
                x_data,
                0,
                y_data,
                where=(left <= x_data) & (x_data <= right),
                interpolate=False,
                color=color,
                alpha=0.5,
                lw=0.2,
        ...
        kdegrid.add_legend(
            label_order=kdegrid.hue_names + ["median"],
            title="",
    ...
    @multi_fit_plot
    def ridgeplot(
        self, df, parameters, row="fit", col="variable", hue="fit", pcolor=-3, height=1
    ):
        """
        ...
        def mykde(x=None, color="purple", label=None):
            ax = sns.kdeplot(
                x, clip_on=False, fill=False, alpha=1, linewidth=1.2, color="w"
            )
            kdeline = ax.lines[0]
            x_data = kdeline.get_xdata()
            y_data = kdeline.get_ydata()
            ...
            ax.vlines(
                median, 0, np.interp(median, y_data, y_data), color="orange", ls=":"
            )
        ...
        with sns.axes_style(style="white", rc={"axes.facecolor": (0, 0, 0, 0)}):
            ...
            pal = sns.cubehelix_palette(
                n_fits + 3, rot=-0.4, start=pcolor, reverse=True
            )
            grid = sns.FacetGrid(
                dmelt,
                row=row,
