...
class SpectraSampler:
    """Simulates binned data sampled from decay spectra."""

    def __init__(
        self,
        funcs_args_rates=None,
        flat_bkg=0,
        f_pileup=0,
        FWHM=0,
        n_events=1e5,
    ...
    def update_bins(self, dE=None, ROI=None, FWHM=None):
        ...
        if FWHM is not None:
            ...
            self.n_window = int(np.ceil(self.sigma / self.dE * 20)) * 2 + 1

        half_window = int(np.floor((self.n_window - 1) / 2))
        if self.response_function:
            response_E = np.linspace(
                -half_window * self.dE,
                half_window * self.dE,
                self.n_window,
                endpoint=True,
            )
            self.conv_window = self.response_function(response_E)
        else:
            self.conv_window = signal.windows.gaussian(
                self.n_window, self.sigma / self.dE
            )
        self.conv_window = self.conv_window / sum(self.conv_window)
        self.bin_edges = np.arange(
            200, self._E_max + self.dE * (half_window + 1), self.dE
        )
        self.n_bins = len(self.bin_edges) - 1

        self.ROI_idx = [
            np.argmin(np.abs(np.array(self.bin_edges) - self.ROI[0])),
            np.argmin(np.abs(np.array(self.bin_edges) - self.ROI[1])),
        ]
        self.ext_ROI_idx = [
            self.ROI_idx[0] - half_window,
            self.ROI_idx[1] + half_window,
        ]

        self.ROI_bin_edges = self.bin_edges[self.ROI_idx[0] : self.ROI_idx[1] + 1]
        self.ROI_bin_centers = (
            self.bin_edges[self.ROI_idx[0] + 1 : self.ROI_idx[1] + 1]
            + self.bin_edges[self.ROI_idx[0] : self.ROI_idx[1]]
        ) / 2
    ...
    def update_spectrum(self, funcs_args_rates=None):
        ...
        for func_name, items in self.funcs_args_rates.items():
            ...
            if self.integrate is True:
                partial_sp = np.array(
                    [
                        integrate.quad(func, edges[i], edges[i + 1], args=tuple(args))[
                            0
                        ]
    ...
    def sample(self, poissonian=True):
        ...
        if poissonian:
            samples = np.random.poisson(
                pdf * self.n_events, size=(self.n_spectra, len(pdf))
            )
        else:
            samples = []
    ...
    def get_measure_time(self, n_det=32):
        ...
        """
        ev_in_ROI = (
            sum(self.full_spectrum[self.ROI_idx[0] : self.ROI_idx[1]]) * n_det * self.dE
        )
